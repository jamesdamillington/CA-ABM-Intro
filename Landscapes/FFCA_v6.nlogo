...
[
  ...
  last_drop            ;;ticks since the last spark was dropped
  count-fires          ;;number fore sparks that caused an ignition
...
to go
  ...
  ;faster section
  let available_space count patches with [pcolor != green]

  ;calculate number of trees to add (technically we could just use spark_freq but this makes things a little clearer in the code)
  let new_trees spark_freq

  ifelse(new_trees < available_space)   ;check there is enough space on the grid to add all these new trees!
  [
    ask n-of new_trees patches with [pcolor != green] [set pcolor green]  ;if there is enough space add all the trees
  ]
  [
    ask patches with [pcolor != green] [set pcolor green]                 ;if there isn't enough space, just turn all remaining space into trees
  ]
...
This model is an implementation of the Forest Fire Cellular Automata described by Bak et al. (1990) 

## HOW IT WORKS

In the FFCA, trees and sparks are dropped in random locations with specified frequency (trees every timestep, sparks every 1/_spark___freq_ timesteps). If a spark falls on a tree the tree burns and the fire spreads (with probability = 1) to any neighbouring trees. 

## HOW TO USE IT

Set the sparking frequency (_spark___freq_), click Setup the click Go. 

## THINGS TO NOTICE

The primary observation of the FFCA is that power-law area-frequency distributions are produced regardless of the frequencies of tree and sparks drops (provided differences in frequencies are relatively large). The distributions are characterised by very many fires of very small area and very few fires with very large fires (i.e. very different from a ‘normal’ distribution). 

## THINGS TO TRY

Experiment with different values of _spark___freq_ to see how a power-law frequency-area distribution is always produced.
...
See the model presented by Peterson (2002).

## CREDITS AND REFERENCES

BAK, P., CHEN, K. & TANG, C. 1990. A forest-fire model and some thoughts on turbulence. _Physics Letters A_, 147, 297–300.

Malamud, B. D., Morein, G., & Turcotte, D. L. (1998). Forest fires: an example of self-organized critical behavior. _Science_, 281(5384), 1840-1842.

Malamud, B. D., Millington, J. D., & Perry, G. L. (2005). Characterizing wildfire regimes in the United States. _Proceedings of the National Academy of Sciences of the United States of America_, 102(13), 4694-4699.
...
<experiments>
  <experiment name="experiment" repetitions="5" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>count-fires = 400</exitCondition>
    <metric>beta</metric>
    <metric>r2</metric>
